...
//stretch back
  while (ros::ok()){
    target_pose = current_pose;
    target_pose.pose.position.x = target_pose.pose.position.x - 0.7;
  ...
  ROS_INFO("Done stretching");
//moving along x in positive direction
  while (ros::ok()){
    target_pose = current_pose;
...
ROS_INFO("Done side 1");
    while (ros::ok()){
    target_pose = current_pose;
    target_pose.pose.position.x = target_pose.pose.position.x + 0.3;
    move_group.setApproximateJointValueTarget(target_pose); // To calculate the trajectory
    move_group.move(); // Move the robot
    current_pose = move_group.getCurrentPose();
    if (abs(current_pose.pose.position.x - target_pose.pose.position.x) < 0.01){
